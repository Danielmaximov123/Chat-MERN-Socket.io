import { Avatar, Box, Divider, ListItemText } from '@mui/material'
    ...
    <Box className="chat-body" >
        {
            ...
            messages?.map((message , index) => {
                return <Box key={index} ref={scroll}
                sx={
                  {padding: '1rem',
                  alignSelf: message.senderId === currentUser ? 'flex-end' : 'flex-start',
                  display: 'inline-grid',
                  border:  message.senderId === currentUser ? '1px #07ae12ce solid' : '1px #696969ab solid',
                  color :  message.senderId === currentUser ? '#07ae12ce' : '#696969ab',
                ...
                >
                  ...
                  <span style={{fontSize: '0.7rem', alignSelf: 'end'}}>{format(message.createdAt)}</span>
                </Box>
            })
            }
            </> 
